...
final class CCValidator: NSObject {
    ...
    convenience init(type: CreditCardType) {
        // default specs
    ...
    private class func dinersClubSpecs() -> (prefixes: [CreditCardPrefix], lengths: [Int]) {
        (
            prefixes: [
                CreditCardPrefix(rangeStart: 300, rangeEnd: 305, length: 3),
    ...
    private class func maestroSpecs() -> (prefixes: [CreditCardPrefix], lengths: [Int]) {
        (
            prefixes: [
                CreditCardPrefix(rangeStart: 50, rangeEnd: 50, length: 2),
    ...
    private class func masterCardSpecs() -> (prefixes: [CreditCardPrefix], lengths: [Int]) {
        (
            prefixes: [
                CreditCardPrefix(rangeStart: 2221, rangeEnd: 2720, length: 4),
    ...
    private class func unionPaySpecs() -> (prefixes: [CreditCardPrefix], lengths: [Int]) {
        (
            prefixes: [
                CreditCardPrefix(rangeStart: 62, rangeEnd: 62, length: 2),
    ...
    private class func visaElectronSpecs() -> (prefixes: [CreditCardPrefix], lengths: [Int]) {
        (
            prefixes: [
                CreditCardPrefix(rangeStart: 4026, rangeEnd: 4026, length: 4),
...
extension CCValidator {
    private func validatePrefixSpecs(creditCardNumber number: String) -> Bool {
        ...
        for prefix in prefixes where number.count >= prefix.prefixLength {
    ...
    class func typeCheckingPrefixOnly(creditCardNumber number: String) -> CreditCardType {
        typeCheckingPrefixOnly(creditCardNumber: number, checkOnlyFromTypes: allTypes())
    }

    class func typeCheckingPrefixOnly(
        creditCardNumber number: String,
        checkOnlyFromTypes types: [CreditCardType]
    ) -> CreditCardType {
    ...
    class func validate(
        ...
        validatePrefix: Bool,
        validateLength: Bool,
        useLuhnAlgorithm useLuhn: Bool
    ) -> Bool {
        let number = number.removingWhitespaceAndNewlines()
        let possibleTypesByLength = possibleTypesCheckingLengthOnly(creditCardNumber: number)
        if validateLength
            && possibleTypesByLength.isEmpty {
            return false
        }
        let typeByPrefix = typeCheckingPrefixOnly(creditCardNumber: number)
        if validatePrefix
        ...
        if useLuhn
            && validateWithLuhnAlgorithm(creditCardNumber: number) == false {
            return false
        }
        return true
    }
